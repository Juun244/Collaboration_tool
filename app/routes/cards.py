...
def create_card(project_id):
    ...
    if "status" not in data:
        logger.error("Missing card status in request")
        return jsonify({"message": "카드 상태가 필요합니다."}), 400

    # 상태 값 유효성 검사
    valid_statuses = ["todo", "in_progress", "done"]
    if data["status"] not in valid_statuses:
        logger.error(f"Invalid card status: {data['status']}")
        return jsonify({"message": "유효하지 않은 카드 상태입니다."}), 400
    ...
    card = {
        ...
        "order": mongo.db.cards.count_documents({"project_id": oid})
    }
    
    result = mongo.db.cards.insert_one(card)
    card["_id"] = result.inserted_id
    
    logger.info(f"Created new card: {card['_id']} in project {project_id}")
    return jsonify({
        "id": str(card["_id"]),
        "title": card["title"],
        "description": card["description"],
